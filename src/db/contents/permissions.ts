...
//
import { listPerfil } from './perfils';
...
const permissions = async (): Promise<void> => {
		...
		if (index !== 0) {
			for (let i = 2; i <= 4; i++) {
				data.push({
					...
					id_perfil: i, //todos los de admision
				});
			}
		}
	});
	...
	//God
	listPerfil.map((perfil: fm_perfil, iPerfil: number) => {
		if (iPerfil !== 0)
			listRoles.map((rol: fm_roles, iRol: number) => {
				if (iRol !== 0)
					data.push({
						id_department: 8, //God
						id_rol: iRol + 1, //todos
						id_perfil: iPerfil + 1, //todos
					});
			});
	});

	//Presidencia
	listPerfil.map((item: fm_perfil, index: number) => {
		if (index !== 0 && (index < 5 || index > 7))
			data.push({
				id_department: 10,
				id_rol: 4, //administrativo
				id_perfil: index + 1, //todos
			});
	});
